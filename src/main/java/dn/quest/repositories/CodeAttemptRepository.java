...
public interface CodeAttemptRepository extends JpaRepository<CodeAttempt, Long> {
    ...
    @Query("SELECT COUNT(ca) > 0 FROM CodeAttempt ca " +
            "WHERE ca.session.id = :sessionId AND ca.level = :level AND ca.submittedNormalized = :normalized")
    boolean existsBySessionAndSubmittedNormalized(Long sessionId, Level level, String normalized);
