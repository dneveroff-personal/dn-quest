-- Создание таблицы для метаданных файлов
...
-- Создание составного индекса для поиска временных истекших файлов
CREATE INDEX idx_file_metadata_temporary_expires ON file_metadata(is_temporary, expires_at) WHERE is_temporary = TRUE;

-- Добавление ограничений
ALTER TABLE file_metadata ADD CONSTRAINT chk_file_metadata_file_size 
    CHECK (file_size > 0);
    
ALTER TABLE file_metadata ADD CONSTRAINT chk_file_metadata_download_count 
...
COMMENT ON COLUMN file_metadata.storage_path IS 'Путь к файлу в хранилище';
COMMENT ON COLUMN file_metadata.description IS 'Описание файла';
COMMENT ON COLUMN file_metadata.owner_id IS 'ID владельца файла';
...
COMMENT ON COLUMN file_metadata.last_accessed_at IS 'Время последнего доступа к файлу';
...
COMMENT ON COLUMN file_metadata.metadata_json IS 'Дополнительные метаданные в JSON формате';
COMMENT ON COLUMN file_metadata.created_at IS 'Время создания записи';
COMMENT ON COLUMN file_metadata.updated_at IS 'Время обновления записи';
