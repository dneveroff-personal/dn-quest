...
public class KafkaEventConsumer {
    ...
    /**
     * Обработка пользовательских событий из Authentication Service
     */
    @KafkaListener(
    ...
    /**
     * Обработка игровых событий из Game Engine Service
     */
    @KafkaListener(
    ...
    /**
     * Обработка файловых событий из File Storage Service
     */
    @KafkaListener(
    ...
    private void handleUserRegistered(UserRegisteredEvent event) {
        ...
        try {
            ...
            userStatisticsService.createUserStatistics(event.getUserId());
            
            log.info("Created profile for new user: userId={}, username={}", 
    ...
    private void handleUserUpdated(UserUpdatedEvent event) {
        ...
        try {
            ...
            userProfileService.updateUserProfileFromEvent(event);
            
            log.info("Updated user profile: userId={}, username={}", 
                    event.getUserId(), event.getUsername());
    ...
    private void handleUserDeleted(UserDeletedEvent event) {
        ...
        try {
            ...
            userSettingsService.deleteUserSettings(event.getUserId());
    ...
    private void handleGameSessionStarted(GameSessionStartedEvent event) {
        log.debug("Handling game session started event for session: {}, user: {}", 
                event.getSessionId(), event.getUserId());
        
        try {
            // Обновляем статистику игровых сессий
            userStatisticsService.updateGameSessionStatistics(event.getUserId(), event.getSessionId(), "STARTED");
            
            // Обновляем время последней активности
            userProfileService.updateLastActivity(event.getUserId());
            
        } catch (Exception e) {
            log.error("Error handling game session started event for session: {}, user: {}", 
    ...
    private void handleGameSessionFinished(GameSessionFinishedEvent event) {
        log.debug("Handling game session finished event for session: {}, user: {}", 
                event.getSessionId(), event.getUserId());
        
        try {
            // Обновляем статистику игровых сессий
            userStatisticsService.updateGameSessionStatistics(event.getUserId(), event.getSessionId(), "FINISHED");
            
            // Обновляем время последней активности
            userProfileService.updateLastActivity(event.getUserId());
            
        } catch (Exception e) {
            log.error("Error handling game session finished event for session: {}, user: {}", 
                    event.getSessionId(), event.getUserId(), e);
            throw e;
        }
    }
    ...
    private void handleLevelCompleted(LevelCompletedEvent event) {
        ...
        try {
            // Обновляем статистику прохождения уровней
            userStatisticsService.updateLevelCompletionStatistics(event.getUserId(), event.getLevelNumber());
            
            // Обновляем время последней активности
            userProfileService.updateLastActivity(event.getUserId());
            
        } catch (Exception e) {
            log.error("Error handling level completed event for session: {}, user: {}, level: {}", 
                    event.getSessionId(), event.getUserId(), event.getLevelNumber(), e);
            throw e;
        }
    }
    ...
    private void handleFileUploaded(FileUploadedEvent event) {
        ...
        try {
            ...
            userStatisticsService.updateFileStatistics(event.getUserId(), event.getFileId(), "UPLOADED");
    ...
    private void handleFileDeleted(FileDeletedEvent event) {
        ...
        try {
            // Обновляем статистику файлов
            userStatisticsService.updateFileStatistics(event.getUserId(), event.getFileId(), "DELETED");
